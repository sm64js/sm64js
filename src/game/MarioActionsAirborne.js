...
const update_air_with_turn = (m) => {
    ...
    // if (!check_horizontal_wind(m)) {
        ...
        if (m.input & Mario.INPUT_NONZERO_ANALOG) {
            ...
            if (m.action == Mario.ACT_PARACHUTING) intendedMag *= 3.0
...
const common_air_action_step = (m, landAction, animation, stepArg) => {
    ...
    if (m.action == Mario.ACT_PARACHUTING) update_air_with_turn(m)
...
const act_freefall = (m) => {
    ...
    switch (m.actionArg) {
        ...
        default: animation = Mario.MARIO_ANIM_GENERAL_FALL; break //throw "act freefall unknown action arg: " + m.actionArg
    }
