...
import { stop_and_set_height_to_floor, perform_air_step, mario_bonk_reflection } from "./MarioStep"
...
const let_go_of_ledge = (m) => {
    ...
    m.vel[1] = 0.0;
    m.forwardVel = -8.0;
    m.pos[0] -= 60.0 * Math.sin(m.faceAngle[1]);
    m.pos[2] -= 60.0 * Math.cos(m.faceAngle[1]);

    floorHeight = SurfaceCollision.find_floor(m.pos[0], m.pos[1], m.pos[2], floor);
...
const act_ledge_grab = (m) => {
    ...
    let hasSpaceForMario = (m.ceilHeight - m.floorHeight >= 160.0);
    ...
    if (m.input & Mario.INPUT_UNKNOWN_10) {
        if (m.marioObj.oInteractStatus & Interact.INT_STATUS_MARIO_UNK1) {
    ...
    heightAboveFloor = m.pos[1] - SurfaceCollision.find_floor_height_relative_polar(m, -0x8000, 30.0);
...
const update_ledge_climb_camera = (m) => {
    ...
    m.statusForCamera.pos[0] = m.pos[0] + sp4 * Math.sin(m.faceAngle[1]);
    m.statusForCamera.pos[2] = m.pos[2] + sp4 * Math.cos(m.faceAngle[1]);
