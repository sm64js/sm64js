...
const interact_player = (m, o) => {
    ...
    // attacked
    let isInCutscene = ((m.action & ACT_GROUP_MASK) == ACT_GROUP_CUTSCENE) || ((m2.action & ACT_GROUP_MASK) == ACT_GROUP_CUTSCENE)
    isInCutscene = isInCutscene || (m.action == ACT_IN_CANNON) || (m2.action == ACT_IN_CANNON)
    const isInvulnerable = (m2.action & ACT_FLAG_INVULNERABLE) || m2.invincTimer != 0 || m2.hurtCounter != 0 || isInCutscene
    const isIgnoredAttack = (m.action == ACT_JUMP || m.action == ACT_DOUBLE_JUMP)
...
const bounce_back_from_attack = (m, interaction) => {
    ...
    if (interaction & (INT_PUNCH | INT_KICK | INT_TRIP)) {
        ...
        //set_camera_shake_from_hit(_ATTACK) TODO
        //if (m.marioObj.localMario) { set_camera_shake_from_hit(_ATTACK); }
...
export const take_damage_and_knock_back = (m, o) => {
    ...
    if (!sInvulnerable && !(m.flags & MARIO_VANISH_CAP)
        ...
        return drop_and_set_mario_action(m, determine_knockback_action(m), damage)
